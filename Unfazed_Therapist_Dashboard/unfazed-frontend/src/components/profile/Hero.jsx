...
export default function Hero({ title = 'Your practice, in balance.', profileImage = '', specializations = [], languages = [] }) {
  ...
  return <section className="relative overflow-hidden rounded-[32px] bg-sage p-7 sm:p-10 lg:p-14">
    <div className="relative z-10 max-w-2xl">
      <div className="h-16 w-16 overflow-hidden rounded-3xl bg-ink text-xl font-bold text-sage">
        {profileImage ? <img src={profileImage} alt={`${title} profile`} className="h-full w-full object-cover" /> : <div className="flex h-full w-full items-center justify-center">{initials}</div>}
      </div>
